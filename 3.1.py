                        ...
                        if numf==numm:
                            for p in range(self.x-1, self.x+2):
                                for q in range(self.y-1, self.y+2):
                                    if p>=0 and q>=0:
                                        try:
                                            if di[(p,q)]['state']=='normal' and di[(p,q)]['text']!='🚩':
                                                di[(p,q)].invoke()
                                        except:
                                            pass
    def mid(self,event):
...
def easy():
    ...
    mineleft=Label(root, text=f'{mine_num}', font=('黑体',16))
    mineleft.place(relx=0.0, rely=0.0)
    
    timecount = Label(root, text='0', font=("黑体",16), anchor='e')
...
def custom1():
    ...
    l0=Label(root, text='Custom Mineboard', font=('黑体',20))
    l0.pack()
    ...
    e3=Entry(root, font=('黑体',15))
    e3.place(relx=0.35, rely=0.69, relwidth=0.3)
    
    b=Button(root, text='Confirm', font=('黑体',15), command=lambda: custom2(e1.get(),e2.get(),e3.get()))
